...
class Database:
    ...
    async def save_registry(self, data: Dict) -> int:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            if existing:
                ...
                await db.execute(
                    """UPDATE registries 
                       SET total_amount = ?, commission = ?, payments_count = ?, 
                           status = ?, tax_file = ?, payments_file = ?
                       WHERE id = ?""",
                    (
                        data["total_amount"],
                        data["commission"],
            ...
            await db.commit()
            logger.info(f"Registry saved: date={data['date']}, id={registry_id}, amount={data['total_amount']}")
            return registry_id
    ...
    async def get_all_time_stats(self) -> Dict:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                """SELECT 
                    COUNT(CASE WHEN total_amount > 0 THEN 1 END) as registries_count,
                    SUM(total_amount) as total_income,
                    SUM(commission) as total_commission,
                    SUM(payments_count) as total_payments
                   FROM registries"""
